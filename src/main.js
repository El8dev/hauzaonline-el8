...
import '../src_old/presentation/controllers/ExamTakerController.js';
    ...
    class AppViewManager {
      constructor() {
        ...
        this.submitMembershipUseCase = new window.SubmitMembershipUseCase(this.studentRepository);
        this.approveStudentUseCase = new window.ApproveStudentUseCase(this.studentRepository);
        this.verifyStudentUseCase = new window.VerifyStudentUseCase(this.studentRepository);
        ...
        window.toggleCreatorCustomSubject = (val) => this.toggleCreatorCustomSubject(val);
        window.closeExamDetailsModal = () => this.closeExamDetailsModal();
        window.openExamDetails = (examId, name, phone) => this.openExamDetails(examId, name, phone);
        window.renderStudentsCumulativeRegistry = () => this.renderStudentsCumulativeRegistry();
      ...
      getSectionsForStage(stageName) {
        if (!stageName) return [];
        let stageSectionsMap = JSON.parse(localStorage.getItem("mzmz_stage_sections_map") || "{}");
      ...
      saveSectionsForStage(stageName, sectionsArray) {
        ...
        localStorage.setItem("mzmz_stage_sections_map", JSON.stringify(stageSectionsMap));
      }

      populateTargetDropdowns(selectedStageForCheckboxes = null) {
        const stageSelect = document.getElementById("creator-target-stage");
        const checkboxesContainer = document.getElementById("creator-target-section-checkboxes");
        const subjectSelect = document.getElementById("creator-subject");
        
        let savedStages = JSON.parse(localStorage.getItem("mzmz_custom_stages") || "[]");
        let savedSubjects = JSON.parse(localStorage.getItem("mzmz_custom_subjects") || "[]");
        
        const defaultStages = ["مرحلة اولى", "مرحلة ثانية", "مرحلة ثالثة", "مرحلة رابعة"];
        ...
        if (stageSelect && stageSelect.options.length <= 1) { // Only populate once
          stageSelect.innerHTML = `<option value="" disabled selected>-- اختر المرحلة --</option>` + allStages.map(s => `<option value="${escapeHtml(s)}">${escapeHtml(s)}</option>`).join("");
        ...
        if (subjectSelect && subjectSelect.options.length <= 1) {
          subjectSelect.innerHTML = `<option value="" disabled selected>-- اختر المادة --</option>` + allSubjects.map(s => `<option value="${escapeHtml(s)}">${escapeHtml(s)}</option>`).join("");
        }
        
        if (checkboxesContainer && selectedStageForCheckboxes) {
          const stageSections = this.getSectionsForStage(selectedStageForCheckboxes);
          checkboxesContainer.innerHTML = stageSections.map(s => `
          ...
          checkboxesContainer.innerHTML = '<span class="text-muted" style="font-size:0.85rem;">يرجى اختيار المرحلة أولاً لعرض الشعب.</span>';
        }
      }
      ...
      loadAdminStructureSettings() {
        ...
        const createListItem = (name, type, extraData = null) => {
          ...
          let dragAttr = type === 'stage' ? `draggable="true" data-index="${allStages.indexOf(name)}"` : "";
        ...
        if (subjectsList) subjectsList.innerHTML = allSubjects.map(s => createListItem(s, 'subject')).join("");
        if (stagesList) {
          stagesList.innerHTML = allStages.map(s => createListItem(s, 'stage')).join("");
        ...
        if (stageFilter) {
          ...
          stageFilter.innerHTML = `<option value="" disabled selected>-- اختر المرحلة لعرض شعبها --</option>` + allStages.map(s => `<option value="${escapeHtml(s)}">${escapeHtml(s)}</option>`).join("");
          ...
          const renderSectionsList = () => {
             ...
             if (!selectedStage) {
               if(sectionsList) sectionsList.innerHTML = '<li class="text-muted" style="font-size:0.85rem;">يرجى اختيار المرحلة أولاً لعرض الشعب.</li>';
               return;
             }
             const stageSections = this.getSectionsForStage(selectedStage);
             if(sectionsList) sectionsList.innerHTML = stageSections.map(s => createListItem(s, 'section', selectedStage)).join("");
      ...
      deleteStructureItem(type, name, extraData = null) {
        ...
        let key = "";
        let defaults = [];
        if(type === 'subject') { key = "mzmz_custom_subjects"; defaults = ["العقائد", "الفقه", "المنطق", "الأخلاق", "النحو"]; }
        if(type === 'stage') { key = "mzmz_custom_stages"; defaults = ["مرحلة اولى", "مرحلة ثانية", "مرحلة ثالثة", "مرحلة رابعة"]; }
      ...
      showNotificationModal({ title = "إشعار", message = "", type = "success", copyText = null, badgeValue = null, onConfirm = null }) {
        ...
        if (valToCopy) {
          ...
          copyBtn.onclick = () => {
            navigator.clipboard.writeText(String(valToCopy)).then(() => {
      ...
      updateTeacherMenuVisibility() {
        ...
        const btnAdminGateTrigger = document.getElementById("btn-admin-gate-trigger");
      ...
      initDropdownMenu() {
        ...
        document.getElementById("menu-item-about").addEventListener("click", (e) => {
          e.preventDefault();
        ...
        document.getElementById("menu-item-teacher").addEventListener("click", (e) => {
          e.preventDefault();
        ...
        if (creatorsModal && document.getElementById("menu-item-creators")) {
          document.getElementById("menu-item-creators").addEventListener("click", (e) => {
            e.preventDefault();
      ...
      initEventListeners() {
        ...
        const eliteModal = document.getElementById("elite-galaxy-modal") || document.getElementById("creators-modal");
        ...
        if (mottoInput) {
          mottoInput.value = localStorage.getItem("mzmz_exam_header_motto") || defaultMotto;
        }
        if (saveMottoBtn) {
          saveMottoBtn.addEventListener("click", () => {
            if (mottoInput && mottoInput.value.trim() !== "") {
              localStorage.setItem("mzmz_exam_header_motto", mottoInput.value.trim());
              alert("✅ تم حفظ وتطبيق النبذة بنجاح! ستظهر الآن بثبات أعلى جميع اختبارات الطلاب.");
            } else {
        ...
        if (authLoginForm) {
          authLoginForm.addEventListener("submit", (e) => {
            ...
            if (email && password) {
              const authEmail = email.includes("@") ? email : `${email}@hawza.local`;
              this.authController.signIn(authEmail, password);
            } else {
        ...
        document.getElementById("role-student-card").addEventListener("click", async () => {
        ...
        document.getElementById("onboard-back-btn").addEventListener("click", () => {
          if (this.currentOnboardStep > 1) {
            this.currentOnboardStep--;
            this.updateOnboardSteps();
          }
        ...
        document.getElementById("go-to-verify-membership-onboard-btn").addEventListener("click", (e) => {
          e.preventDefault();
          this.showStudentCard("student-verify-card");
        });

        document.getElementById("role-teacher-card").addEventListener("click", () => {
          document.getElementById("global-nav").style.display = "flex";
          this.authController.checkSession();
        });

        document.getElementById("student-back-home").addEventListener("click", () => {
        ...
        document.querySelectorAll(".sort-student-btn").forEach(btn => {
          btn.addEventListener("click", (e) => {
             ...
             const order = (currentSort.by === sortType && currentSort.order === 'asc') ? 'desc' : 'asc';
        ...
        document.getElementById("tab-registry-btn").addEventListener("click", () => {
        ...
        document.getElementById("tab-attendance-btn").addEventListener("click", () => {
          switchAdminTab("tab-attendance-content", "tab-attendance-btn");
          this.loadAdminAttendanceSettings();
          this.loadAdminAttendanceTable();
        });

        document.getElementById("tab-structure-btn")?.addEventListener("click", () => {
        ...
        document.getElementById("btn-admin-add-stage")?.addEventListener("click", () => {
          const input = document.getElementById("admin-new-stage");
          this.addStructureItem('stage', input.value);
          input.value = "";
        });

        document.getElementById("btn-admin-add-section")?.addEventListener("click", () => {
        ...
        const attendanceActiveCb = document.getElementById("admin-attendance-active");

        const attendanceModeSelect = document.getElementById("admin-attendance-mode");
        const attendanceCustomDays = document.getElementById("admin-attendance-custom-days");
        const btnSaveAttendance = document.getElementById("btn-save-attendance-settings");

        const attendanceTimeModeSelect = document.getElementById("admin-attendance-time-mode");
        const attendanceCustomTime = document.getElementById("admin-attendance-custom-time");
        ...
        if (btnSaveAttendance) {
          btnSaveAttendance.addEventListener("click", () => {
            ...
            const startTime = document.getElementById("admin-attendance-start-time")?.value || "";
            const endTime = document.getElementById("admin-attendance-end-time")?.value || "";
            const settings = { active, mode, selectedDays, timeMode, startTime, endTime };
            localStorage.setItem("mzmz_attendance_settings", JSON.stringify(settings));
        ...
        if (btnRegisterAttendance) {
          btnRegisterAttendance.addEventListener("click", () => {
             ...
             const records = JSON.parse(localStorage.getItem("mzmz_attendance_records") || "[]");
             
             // Add record if not exists
             const exists = records.find(r => r.studentId === this.currentStudent.studentId && r.date === today);
             if (!exists) {
               records.push({
               ...
               localStorage.setItem("mzmz_attendance_records", JSON.stringify(records));
             }
             
             document.getElementById("student-attendance-banner").style.display = "none";
             document.getElementById("student-attendance-success").style.display = "block";
          });
        }
        ...
        document.getElementById("go-to-register-membership-btn").addEventListener("click", (e) => {
          e.preventDefault();
          this.showStudentCard("student-onboarding-card");
          this.currentOnboardStep = 1;
          this.updateOnboardSteps();
        ...
        document.getElementById("go-to-verify-membership-btn").addEventListener("click", (e) => {
          e.preventDefault();
          this.showStudentCard("student-verify-card");
        });
        ...
        const academicDeptContainer = document.getElementById("req-academic-dept-container");
        ...
        const hawzaDescContainer = document.getElementById("req-hawza-desc-container");
        ...
        document.getElementById("student-request-form").addEventListener("submit", async (e) => {
          e.preventDefault();
          const firstName = document.getElementById("req-student-first-name").value.trim();
          const fatherName = document.getElementById("req-student-father-name").value.trim();
          const grandName = document.getElementById("req-student-grand-name").value.trim();
          const name = `${firstName} ${fatherName} ${grandName}`.trim();
          
          const surname = document.getElementById("req-student-surname").value.trim();
          const birthdate = document.getElementById("req-student-birth").value.trim();
          const province = document.getElementById("req-student-province").value.trim();
          ...
          const hawzaDesc = document.getElementById("req-hawza-desc").value.trim();
          ...
          try {
            ...
            this.hideLoading();
            alert("✉️ تم تقديم طلبك التحاقك بنجاح! يرجى الانتظار حتى تقوم الإدارة بقبول الطلب وتزويدك بالرقم الحوزوي.");
        ...
        document.getElementById("student-verify-form").addEventListener("submit", async (e) => {
          e.preventDefault();
          
          const name = document.getElementById("student-verify-name").value.trim();
          const studentId = document.getElementById("student-verify-id").value.trim();

          this.showLoading();
          try {
            // 1. التحقق من بيانات الطالب
            const student = await this.studentRepository.loginStudent(name, studentId);
            if (!student) {
              throw new Error("بيانات الطالب (الاسم أو العضوية) غير صحيحة أو غير معتمدة.");
            }
            ...
            localStorage.setItem("MZMZ_STUDENT_SESSION", JSON.stringify({ name: name, id: studentId }));
        ...
        const studentExamsLogout = document.getElementById("student-logout-from-exams");
        ...
        const successBackBtn = document.getElementById("success-back-to-portal-btn");
        if (successBackBtn) {
          successBackBtn.addEventListener("click", () => {
            if (this.currentStudent) {
              this.switchView("view-student-entry");
              document.getElementById("student-verify-card").style.display = "none";
              document.getElementById("student-exams-list-card").style.display = "block";
              this.loadStudentExamsPortal().catch(err => {
                 this.showError(err.message);
                 this.currentStudent = null;
                 document.getElementById("student-exams-list-card").style.display = "none";
                 document.getElementById("student-verify-card").style.display = "block";
                 document.getElementById("student-verify-form").reset();
              });
            } else {
              this.switchView("view-role-selection");
      ...
      restoreDefaultView(updateHash = true) {
        ...
        if (authorized) {
          ...
          this.showStudentCard("student-onboarding-card", updateHash);
          this.currentOnboardStep = 1;
          this.updateOnboardSteps();
        }
      }
      ...
      async initRouting() {
        ...
        if (examId) {
          ...
          try {
            ...
            document.getElementById("entry-exam-title").textContent = "امتحان: " + examData.title;
            document.getElementById("entry-exam-desc").textContent = examData.description || "أدخل رقم عضويتك المعتمد لحل الأسئلة.";
      ...
      loadAdminAttendanceSettings() {
        const settings = JSON.parse(localStorage.getItem("mzmz_attendance_settings") || '{"active":false,"mode":"all","selectedDays":[]}');
        ...
        if (customDiv) {
          ...
          document.querySelectorAll(".attendance-day-cb").forEach(cb => {
            cb.checked = settings.selectedDays && settings.selectedDays.includes(parseInt(cb.value));
          });
        }
        ...
        const customTimeDiv = document.getElementById("admin-attendance-custom-time");
        if (customTimeDiv) {
          customTimeDiv.style.display = selTimeMode && selTimeMode.value === "customtime" ? "flex" : "none";
        }
        
        const startTimeInput = document.getElementById("admin-attendance-start-time");
      ...
      async loadAdminAttendanceTable() {
        ...
        try {
          ...
          const approvedStudents = studentsList.filter(s => s.status === "approved");
          const records = JSON.parse(localStorage.getItem("mzmz_attendance_records") || "[]");
          ...
          if (stageFilter && stageFilter.options.length <= 1) {
            const uniqueStages = [...new Set(approvedStudents.map(s => s.stage).filter(Boolean))];
          ...
          if (sectionFilter && sectionFilter.options.length <= 1) {
            const uniqueSections = [...new Set(approvedStudents.map(s => s.qualification).filter(Boolean))];
          ...
          const filteredStudents = approvedStudents.filter(student => {
            const matchStage = selectedStage === "الكل" || student.stage === selectedStage;
            const matchSection = selectedSection === "الكل" || student.qualification === selectedSection;
          ...
          filteredStudents.forEach(student => {
            ...
            const presences = records.filter(r => r.studentId === student.id).length;
            const absences = Math.max(0, totalActiveDays - presences);
            
            const hawzaNumber = student.member_number || student.hawza_number || "غير محدد";
      ...
      onSupabaseNotConfigured() {
        ...
        this.currentUserId = null;
        document.getElementById("nav-logout").style.display = "none";
        alert("لم يتم الاتصال بقاعدة البيانات (Supabase). يرجى التأكد من إضافة الرابط والمفتاح في الإعدادات البرمجية.");
        this.switchView("view-auth");
      }
      ...
      async loadStudentExamsPortal() {
        ...
        try {
          ...
          const attendanceSettings = JSON.parse(localStorage.getItem("mzmz_attendance_settings") || '{"active":false}');
          const studentBanner = document.getElementById("student-attendance-banner");
          const studentSuccess = document.getElementById("student-attendance-success");
          ...
          if (studentBanner && studentSuccess) {
            ...
            if (attendanceSettings.active) {
              ...
              } else if (attendanceSettings.mode === "custom" && attendanceSettings.selectedDays && attendanceSettings.selectedDays.includes(todayDayOfWeek)) {
                isRequiredToday = true;
              }
              
              if (isRequiredToday) {
                if (attendanceSettings.timeMode === "customtime" && (attendanceSettings.startTime || attendanceSettings.endTime)) {
                  const nowStr = todayDate.getHours().toString().padStart(2, '0') + ":" + todayDate.getMinutes().toString().padStart(2, '0');
                  if (attendanceSettings.startTime && nowStr < attendanceSettings.startTime) isRequiredToday = false;
                  if (attendanceSettings.endTime && nowStr > attendanceSettings.endTime) isRequiredToday = false;
                }
              }
              
              if (isRequiredToday) {
                const records = JSON.parse(localStorage.getItem("mzmz_attendance_records") || "[]");
                const hasAttended = records.some(r => r.studentId === student.studentId && r.date === todayStr);
          ...
          const submissions = await subRepo.getSubmissionsByStudent(student.studentPhone, student.studentName);
          ...
          submissions.forEach(sub => {
            ...
            if (relatedExam) {
                ...
                if (type === "final" || type === "half") {
                    subjectTotalScores[relatedExam.subject] = (subjectTotalScores[relatedExam.subject] || 0) + sub.score;
                }
            }
          });
          ...
          const availableExams = allExams.filter(exam => {
            ...
            const isSecondSession = (exam.test_type === "second_session" || exam.testType === "second_session");
            if (isSecondSession) {
          ...
          const completedExams = allExams.filter(exam => submittedExamIds[exam.id]);
          ...
          if (availableExams.length === 0) {
            ...
            if (container) {
              container.innerHTML = availableExams.map(exam => {
                const subjectTag = exam.subject && exam.subject !== 'غير محدد' 
          ...
          if (completedContainer) {
            if (completedExams.length === 0) {
              ...
              completedContainer.innerHTML = completedExams.map(exam => {
                const subjectTag = exam.subject && exam.subject !== 'غير محدد' 
                  ? `<span class="badge primary" style="font-size: 0.75rem; margin-inline-start: 0.5rem; vertical-align: middle;">📚 ${escapeHtml(exam.subject)}</span>` 
                  : '';
      ...
      renderExamsList(exams) {
        ...
        exams.forEach(exam => {
          document.getElementById(`btn-results-${exam.id}`).addEventListener("click", () => {
            this.creatorController.loadExamResults(exam.id);
          });
          document.getElementById(`btn-delete-${exam.id}`).addEventListener("click", () => {
      ...
      async loadStudentsList() {
        ...
        try {
          ...
          const exams = await this.creatorController.examRepository.listExamsByCreator(this.currentUserId);
          const submissionsPromises = exams.map(exam => this.creatorController.submissionRepository.getSubmissionsByExam(exam.id));
          ...
          list.sort((a, b) => {
             ...
             switch (currentSort.by) {
               case 'name': 
                 valA = a.student_name; valB = b.student_name; break;
               case 'hawza_number': 
                 valA = a.member_number || a.hawza_number || 0; valB = b.member_number || b.hawza_number || 0; break;
               case 'stage': 
                 valA = a.stage; valB = b.stage; break;
               case 'status': 
                 valA = a.status; valB = b.status; break;
               case 'avg_score': 
                 valA = a.avg_score; valB = b.avg_score; break;
               case 'date': 
               default: 
          ...
          const savedStages = JSON.parse(localStorage.getItem("mzmz_custom_stages") || "[]");
          const allStages = [...new Set([...defaultStages, ...savedStages])];

          const stagesOpts = allStages.map(s => `<option value="${escapeHtml(s)}">${escapeHtml(s)}</option>`).join("");
          ...
          if (pending.length === 0) {
            ...
            pendContainer.innerHTML = pending.map(p => `
            ...
            pending.forEach(p => {
              document.getElementById(`btn-approve-${p.id}`).addEventListener("click", async () => {
                const stage = document.getElementById(`sel-stage-${p.id}`).value;
                const section = document.getElementById(`sel-sec-${p.id}`).value;
                const hawzaInput = document.getElementById(`inp-hawza-${p.id}`).value;
              ...
              document.getElementById(`btn-reject-${p.id}`).addEventListener("click", async () => {
            ...
            approved.forEach(a => {
              ...
              if (promoteBtn) {
                promoteBtn.addEventListener("click", async () => {
                  ...
                  if (currentStageIdx === -1 || currentStageIdx >= allStages.length - 1) {
                    alert("الطالب في المرحلة النهائية أو مرحلته غير معروفة، لا يمكن ترقيته أكثر.");
                    return;
                  }
                  ...
                  if (!confirm(`هل أنت متأكد من ترقية الطالب من '${a.stage}' إلى '${nextStage}'؟`)) return;
              ...
              document.getElementById(`btn-del-stud-${a.id}`).addEventListener("click", async () => {
                if (!confirm("هل أنت متأكد من حذف عضوية هذا الطالب؟ الرقم المخصص له سيبقى محجوزاً للتاريخ.")) return;
                this.showLoading();
                try {
              ...
              document.getElementById(`btn-edit-hawza-${a.id}`).addEventListener("click", async () => {
              ...
              document.getElementById(`btn-save-sec-${a.id}`).addEventListener("click", async () => {
                const stage = document.querySelector(`.sel-edit-stage[data-id="${a.id}"]`).value;
                const sec = document.querySelector(`.sel-edit-sec[data-id="${a.id}"]`).value;
                this.showLoading();
                try {
                  await this.studentRepository.updateStudentStageAndSection(a.id, stage, sec);
                  this.hideLoading();
      ...
      renderExamCreator() {
        ...
        const distRadios = document.querySelectorAll('input[name="grade_distribution"]');
        if (totalGradeInput) {
          totalGradeInput.addEventListener("input", () => this.recalculateCreatorGrades());
        }
        ...
        document.querySelectorAll(".creator-question-box").forEach(box => box.remove());
        
        document.querySelectorAll(".creator-target-section-cb, .creator-section-cb").forEach(cb => cb.checked = false);
        ...
        if (startTimeInput._flatpickr) startTimeInput._flatpickr.setDate(startTimeInput.value);
        if (endTimeInput._flatpickr) endTimeInput._flatpickr.setDate(endTimeInput.value);
      ...
      recalculateCreatorGrades() {
        const distMode = document.querySelector('input[name="grade_distribution"]:checked')?.value || "equal";
        const gradeInputs = document.querySelectorAll(".q-points-input");
        const totalGrade = parseFloat(document.getElementById("creator-total-grade")?.value) || 100;
      ...
      reindexCreatorQuestions() {
        ...
        Array.from(container.children).forEach((box, idx) => {
          ...
          box.querySelectorAll('input[type="checkbox"].q-correct-checkbox').forEach(chk => {
      ...
      async submitCreatedExam() {
        ...
        let targetStage = document.getElementById("creator-target-stage").value.trim();
        
        let targetSections = [];
        document.querySelectorAll(".creator-target-section-cb, .creator-section-cb").forEach(cb => {
        ...
        document.querySelectorAll(".creator-question-box").forEach((box, qIdx) => {
          ...
          const correctIndices = Array.from(correctCheckboxes).map(chk => parseInt(chk.value));
        ...
        const shuffleOrder = document.getElementById("creator-shuffle-order") ? document.getElementById("creator-shuffle-order").checked : true;
        const testType = document.getElementById("creator-test-type") ? document.getElementById("creator-test-type").value : "quiz";

        if (testType === 'half' || testType === 'final' || testType === 'second_session') {
            this.showLoading();
            const allExams = await this.creatorController.examRepository.listAllExams();
            this.hideLoading();
            ...
            for (const exam of allExams) {
               const typeMatch = (exam.test_type === testType || exam.testType === testType);
               const subjectMatch = (exam.subject === subject);
               const stageMatch = (exam.targetStage === targetStage || exam.targetStage === "الكل");
               
               if (!typeMatch || !subjectMatch || !stageMatch) continue;
               
               const existingSections = Array.isArray(exam.targetSections) ? exam.targetSections : (typeof exam.targetSections === 'string' ? JSON.parse(exam.targetSections || '["الكل"]') : ["الكل"]);
               const hasOverlap = existingSections.includes("الكل") || targetSections.includes("الكل") || existingSections.some(s => targetSections.includes(s));
            ...
            if (isDuplicate) {
                const typeLabel = testType === 'final' ? 'نهائي' : (testType === 'half' ? 'نصف السنة' : 'دور ثاني');
                alert(`🚨 لا يمكن إنشاء أكثر من امتحان (${typeLabel}) لنفس المادة والمرحلة والشعبة.`);
                return;
            }
        }
      ...
      onExamCreated(exam) {
        ...
        this.showNotificationModal({
          ...
          copyText: shareUrl,
          onConfirm: () => {
            if (document.getElementById("creator-test-type")) document.getElementById("creator-test-type").value = "quiz";
            document.getElementById("tab-exams-btn").click();
          }
        });
      }
      ...
      renderExamTaker({ exam, questions }) {
        ...
        document.getElementById("taker-student-badge").textContent = `الطالب: ${this.currentStudent.studentName} (عضو رقم: ${this.currentStudent.memberNumber || this.currentStudent.hawzaNumber || ''})`;
        ...
        container.innerHTML = displayedQuestions.map((q, idx) => {
          const isMultiple = q.correctOptions && q.correctOptions.length > 1;
          const inputType = isMultiple ? "checkbox" : "radio";
          // ترتيب الخيارات عشوائياً
          const displayedOptions = shouldShuffle ? shuffleArray(q.options) : [...q.options];
        ...
        displayedQuestions.forEach(q => {
          document.querySelectorAll(`input[name="answer-for-${q.id}"]`).forEach(input => {
            input.addEventListener("change", (e) => {
              if (input.type === "radio") {
                document.querySelectorAll(`[id^="choice-${q.id}-"]`).forEach(lbl => {
                  lbl.classList.remove("selected");
                });
                if (e.target.checked) {
                  document.getElementById(`choice-${q.id}-${e.target.value}`).classList.add("selected");
                }
              } else {
                const lbl = document.getElementById(`choice-${q.id}-${e.target.value}`);
                if (e.target.checked) {
        ...
        document.getElementById("exam-taker-form").addEventListener("submit", (e) => {
          e.preventDefault();
          
          const answers = {};
          displayedQuestions.forEach(q => {
            const checkedInputs = document.querySelectorAll(`input[name="answer-for-${q.id}"]:checked`);
            if (checkedInputs.length > 0) {
              if (checkedInputs[0].type === "radio") {
                answers[q.id] = parseInt(checkedInputs[0].value, 10);
              } else {
                answers[q.id] = Array.from(checkedInputs).map(inp => parseInt(inp.value, 10));
              }
            }
          });
      ...
      onExamNotStarted(exam) {
        ...
        document.getElementById("exam-message-title").textContent = "لم يبدأ الامتحان بعد";
      ...
      onExamEnded(exam) {
        this.hideLoading();
        this.switchView("view-exam-message");
        document.getElementById("exam-message-title").textContent = "انتهى وقت الامتحان";
        document.getElementById("exam-message-body").innerHTML = `
      ...
      onExamNotFound() {
        this.hideLoading();
        this.switchView("view-exam-message");
        document.getElementById("exam-message-title").textContent = "الامتحان غير موجود";
        document.getElementById("exam-message-body").textContent = "عذراً، الرابط غير صحيح أو تم حذف هذا الامتحان من قبل المسؤول.";
      }

      onExamAlreadyTaken(exam, existingSub) {
        this.hideLoading();
        this.switchView("view-exam-message");
        document.getElementById("exam-message-title").textContent = "⚠️ تم أداء هذا الامتحان مسبقاً";
        const studentName = this.currentStudent ? this.currentStudent.studentName : "";
        document.getElementById("exam-message-body").innerHTML = `
      ...
      renderFilteredExamResults() {
        ...
        if (stageFilter || qualFilter) {
          filteredSubmissions = filteredSubmissions.filter(sub => {
            const student = students.find(s => s.phone === sub.studentPhone || s.name === sub.studentName);
        ...
        filteredSubmissions.sort((a, b) => {
          const studentA = students.find(s => s.phone === a.studentPhone || s.name === a.studentName);
          const studentB = students.find(s => s.phone === b.studentPhone || s.name === b.studentName);
        ...
        const rows = filteredSubmissions.map(sub => {
          ...
          const answersCells = questions.map(q => {
      ...
      toggleCreatorCustomSubject(val) {
        document.getElementById("creator-custom-subject-group").style.display = val === "custom" ? "block" : "none";
      }
      ...
      async openExamDetails(examId, studentName, studentPhone) {
        ...
        document.getElementById("details-student-phone").textContent = studentPhone || "غير متوفر";
        document.getElementById("details-exam-title").textContent = exam.title;
        
        const totalPoints = questions.reduce((sum, q) => sum + q.points, 0);
        const scorePercent = totalPoints > 0 ? Math.round((sub.score / totalPoints) * 100) : 0;
        document.getElementById("details-score-badge").innerHTML = `<span style="color:var(--primary-color);">${sub.score} / ${totalPoints}</span> (${scorePercent}%)`;
        ...
        questions.forEach((q, qIdx) => {
          const studentAnsIdx = sub.answers[q.id];
          const isCorrect = studentAnsIdx !== undefined && Number(studentAnsIdx) === q.correctOptionIndex;
          ...
          card.innerHTML = `
            ...
            <div style="display:flex; flex-direction:column; gap:0.5rem;">
              ${q.options.map((opt, oIdx) => {
                ...
                const isSelectedByStudent = studentAnsIdx !== undefined && Number(studentAnsIdx) === oIdx;
    ...
    async renderStudentsCumulativeRegistry() {
      ...
      try {
        ...
        const submissionsPromises = exams.map(exam => 
          this.creatorController.submissionRepository.getSubmissionsByExam(exam.id)
        );
        const submissionsResults = await Promise.all(submissionsPromises);
        const allSubmissions = submissionsResults.flat();
          ...
          exams.forEach(exam => {
            if (exam.isEnded()) {
              ...
              Object.values(registry).forEach(student => {
                ...
                if (halfSub || finalSub) {
                  student.successMeasure = (student.halfGrade || 0) + (student.finalGrade || 0);
    ...
    filterRegistry() {
      ...
      if (searchQuery) {
        students = students.filter(s => s.name.toLowerCase().includes(searchQuery) || s.phone.includes(searchQuery));
      }
      if (stageFilter) {
    ...
    exportRegistryToCSV() {
      ...
      csvContent += "الاسم,رقم الهاتف,المرحلة,الشعبة,عدد الامتحانات,متوسط الدرجة %\n";

      students.forEach(s => {
        const examsCount = s.submissions.length;
        const avgPct = examsCount > 0 ? Math.round(s.submissions.reduce((acc, sub) => acc + sub.score, 0) / examsCount * 10) : 0;
    ...
    async renderLeaderboard() {
      ...
      let students = Object.values(this.currentRegistry).filter(s => s.submissions.length > 0);
    ...
    openPendingProfile(id) {
      ...
      document.getElementById("pp-name").textContent = `${p.student_name} ${p.surname || ''}`;
      document.getElementById("pp-date").textContent = `📅 تاريخ التقديم: ${new Date(p.created_at).toLocaleDateString("ar")}`;
      ...
      newBtnApprove.addEventListener("click", async () => {
        const stage = prompt("أدخل المرحلة الدراسية المخصصة للطالبة:", p.stage && p.stage !== "لم يتم التحديد بعد" ? p.stage : "");
        if (stage === null) return;
        const cleanStage = stage.trim() || "غير محدد";

        const section = prompt("أدخل الشعبة المخصصة للطالبة (مثال: أ، ب، ج):", p.qualification || "");
        if (section === null) return;
        const cleanSection = section.trim() || "غير محدد";
        
        const hawzaInput = prompt("أدخل الرقم الحوزوي (اختياري). إذا تُرك فارغاً سيتم توليد رقم تسلسلي تلقائياً:");
    ...
    showStudentAttendance(studentId, studentName) {
      ...
      let lastDays = studentRecords.slice(-3).map(r => r.date).join(" , ");
    ...
    openStudentProfile(phone) {
      ...
      document.getElementById("sp-stage").textContent = `🏛️ المرحلة: ${student.stage || '—'}`;
      document.getElementById("sp-qual").textContent = `🔖 الشعبة: ${student.qualification || '—'}`;
      document.getElementById("sp-phone").textContent = `🔢 الرقم الحوزوي: ${student.hawza_number || '—'}`;
      
      const elWhatsapp = document.getElementById("sp-whatsapp");
      if (elWhatsapp) elWhatsapp.textContent = `📞 واتساب: ${student.phone}`;
      
      document.getElementById("sp-city").textContent = `📍 المدينة: ${student.city || '—'}`;
      
      const elBirth = document.getElementById("sp-birthdate");
      if (elBirth) elBirth.textContent = `🎂 المواليد: ${student.birthdate || '—'}`;
      
      const elMarital = document.getElementById("sp-marital");
      if (elMarital) elMarital.textContent = `💍 الحالة: ${student.marital_status || '—'}`;
      
      const elStudy = document.getElementById("sp-study");
      if (elStudy) elStudy.textContent = `📚 الدراسة: ${student.study_type || '—'}`;
      
      const elIsStudent = document.getElementById("sp-isstudent");
      if (elIsStudent) elIsStudent.textContent = `🎓 طالبة: ${student.is_student || '—'}`;
      
      // Populate Stats
      document.getElementById("sp-total-exams").textContent = subs.length;
      const avgPct = subs.length > 0 ? Math.round(subs.reduce((acc, sub) => acc + sub.score, 0) / subs.length * 10) : 0;
      ...
      document.getElementById("sp-subjects").innerHTML = subjectsHTML || '<span class="text-muted">لم يدرس أي مادة بعد</span>';
    ...
    openCertificateModal(phone) {
      ...
      const score = student.successMeasure !== undefined ? student.successMeasure : 0;
    ...
    toggleCertLayout() {
      ...
      if (isMinimized) {
        ...
        document.getElementById("cert-content-layer").style.background = "rgba(255, 255, 255, 0.88)";
      } else {
        hSec.style.display = "none";
        if (fSec) fSec.style.display = "none";
        if (pPrayer) pPrayer.style.display = "none";
        if (label) label.style.display = "none";
        document.getElementById("cert-content-layer").style.background = "rgba(255, 255, 255, 0.15)";
    ...
    generateMasterGradesReport() {
      ...
      if (qualFilter) students = students.filter(s => s.qualification === qualFilter);
      ...
      if (elStage) elStage.textContent = `المرحلة: ${stageFilter ? stageFilter : "جميع المراحل"}`;
      if (elGroup) elGroup.textContent = `الشعبة: ${qualFilter ? "شعبة " + qualFilter : "جميع الشعب"}`;
      if (elDate) elDate.textContent = `تاريخ الرصد: ${new Date().toLocaleDateString("ar-EG")}`;
      ...
      if (students.length === 0) {
        ...
        tbody.innerHTML = students.map((s, idx) => {
          ...
          let gradeColor = "red";
          if (succ >= 90) { gradeWord = "امتياز 🌟"; gradeColor = "darkgreen"; }
          else if (succ >= 80) { gradeWord = "جيد جداً"; gradeColor = "green"; }
          else if (succ >= 70) { gradeWord = "جيد"; gradeColor = "blue"; }
          else if (succ >= 50 || s.submissions.length > 0) { gradeWord = "ناجح ومستوفي"; gradeColor = "#0f766e"; }
